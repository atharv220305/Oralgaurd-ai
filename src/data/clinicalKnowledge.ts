...
==================================================`;
  ...
  let identifiedCondition = 'Routine Oral Health Screening';
  if (profile.emergencyFlagTriggered) {
    identifiedCondition = 'Acute Airway or Severe Oral Emergency';
  } else if (profile.neckLumpOrSwelling || profile.persistentHoarseness) {
    identifiedCondition = 'Neck Mass / Persistent Hoarseness';
  } else if (riskScore >= 45 || (profile.durationOverTwoWeeks && (profile.hasLesionOrUlcer || profile.colorChanges === 'red' || profile.reducedMouthOpening))) {
    identifiedCondition = 'Persistent Oral Mucosal Lesion (>14 Days)';
  } else if (hasGumBleeding && !hasDecay) {
    identifiedCondition = 'Periodontal Inflammation / Gingivitis';
  } else if (hasToothPain || hasDecay || hasSensitivity) {
    identifiedCondition = 'Dental Caries / Odontogenic Pain';
  } else if (concerns.length > 0 && concerns[0].title) {
    identifiedCondition = concerns[0].title;
  }
